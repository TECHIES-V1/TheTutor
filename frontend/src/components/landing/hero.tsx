...
export function Hero() {
  return (
    <section className="relative overflow-hidden pb-20 pt-32 md:pb-28 md:pt-40">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(212,175,55,0.22),transparent_32%),radial-gradient(circle_at_82%_20%,rgba(212,175,55,0.14),transparent_34%)]" />
      ...
      <div className="container relative z-10 grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
        ...
        >
          <div className="neo-inset inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold uppercase tracking-[0.14em] text-primary">
            AI Tutor
            <span className="rounded-full bg-primary/15 px-2 py-0.5 text-primary">Personalized</span>
          </div>
          ...
          <h1 className="mt-6 text-balance font-playfair text-5xl font-bold leading-tight text-foreground md:text-6xl lg:text-[4.25rem]">
            Learn Smarter With
            <span className="block bg-gradient-to-r from-[#d4af37] via-[#f2dc92] to-[#8b6f47] bg-clip-text text-transparent">
              TheTutor
            </span>
          </h1>

          <p className="mt-5 max-w-2xl text-lg leading-relaxed text-muted-foreground md:text-xl">
            Build a learning plan that fits your pace, your goals, and your schedule. Get tutor guidance, structured modules, and instant feedback in one premium black and gold workspace.
          </p>

          <div className="mt-8 flex flex-col gap-4 sm:flex-row">
            <Button asChild size="lg" className="skeuo-gold rounded-full px-8 hover:!opacity-100">
            ...
            <Button
              variant="outline"
              size="lg"
              className="neo-inset rounded-full border-primary/20 text-foreground hover:bg-muted hover:text-foreground"
            >
          ...
          <div className="mt-9 grid gap-3 text-sm text-muted-foreground sm:grid-cols-3">
            {["Adaptive paths", "Live feedback", "Skill certificates"].map((item) => (
              <div key={item} className="neo-inset flex items-center gap-2 rounded-xl px-3 py-2">
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            <article className="neo-surface overflow-hidden rounded-3xl sm:row-span-2">
              <img src={heroImages[0].src} alt={heroImages[0].alt} className="h-full w-full object-cover" />
            </article>
            <article className="neo-surface overflow-hidden rounded-3xl">
          ...
          <div className="neo-surface absolute -bottom-5 -left-4 rounded-2xl px-4 py-3">
            ...
            <p className="mt-1 text-xl font-bold text-primary">12,000+</p>
          </div>
        </motion.div>
      </div>
    </section>
