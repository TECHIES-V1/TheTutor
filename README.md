...
| **Joanna Bassey** | Frontend Developer & SEO | Built the initial landing page template (hero section, navbar, features, how-it-works, footer, CTA). Implemented comprehensive SEO infrastructure: full metadata system (`generateMetadata`), dynamic `sitemap.ts`, `robots.ts`, Open Graph image generation route, and JSON-LD structured data (Organization, Website, EducationalOrganization). Created `.env.example` for frontend. Redesigned the sign-in page layout. Built the logout confirmation modal with centered auth pages. Added dynamic copyright year to footer, responsive navbar repositioning, and applied component cleanup and formatting across the codebase. | [DevBytes-J](https://github.com/DevBytes-J) |
