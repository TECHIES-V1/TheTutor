...
const sse_1 = require("../middleware/sse");
const Conversation_1 = require("../models/Conversation");
const Course_1 = require("../models/Course");
require("../models/Resource"); // Ensure it is registered for population
const generator_1 = require("../services/course/generator");
const router = (0, express_1.Router)();
// ── POST /course/generate ─────────────────────────────────────────────────
router.post("/generate", auth_1.requireAuth, sse_1.sseHeaders, async (req, res) => {
    const keepAlive = (0, sse_1.startKeepAlive)(res);
    try {
        ...
        if (!conversationId) {
            ...
            (0, sse_1.stopKeepAlive)(keepAlive);
            (0, sse_1.endSSE)(res);
            return;
        }
        ...
        for await (const event of (0, generator_1.generate)(conversationId, userId)) {
            (0, sse_1.sendSSE)(res, event.type, event.data);
...
router.get("/generation-status/:conversationId", auth_1.requireAuth, async (req, res) => {
    try {
...
router.get("/:id", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const course = await Course_1.Course.findById(id).populate("resources", "title authors source");
        if (!course) {
            res.status(404).json({
...
router.post("/:id/modules/:moduleId/lessons/:lessonId/quizzes/:quizId/questions/:questionId/answer", auth_1.requireAuth, async (req, res) => {
    try {
        ...
        const courseDoc = await Course_1.Course.findById(id);
        if (!courseDoc) {
...
router.put("/:id/progress", auth_1.requireAuth, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const completedLessons = courseDoc.modules.reduce((total, module) => total + module.lessons.filter((l) => l.completed).length, 0);
