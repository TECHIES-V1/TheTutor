...
function buildStubCourse(input) {
...
async function generateWithRemote(input) {
    ...
    try {
        ...
        if (!payload?.title || !Array.isArray(payload.curriculum))
            return null;
        return payload;
    }
    catch {
...
async function generateCourse(input) {
    ...
    if (mode === "remote") {
        ...
        if (remote)
            return { source: "generator", course: remote };
    }
