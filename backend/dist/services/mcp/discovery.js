...
exports.discoverAndParseBooks = discoverAndParseBooks;
const client_1 = require("./client");
const nova_1 = require("../ai/nova");
const Resource_1 = require("../../models/Resource");
async function* discoverAndParseBooks(onboardingData) {
    ...
    const isHealthy = await (0, client_1.checkHealth)();
    if (!isHealthy) {
        yield {
            type: "error",
            data: {
    ...
    const searchResult = await (0, client_1.discoverySearch)({
        query: searchQuery,
        limit: 18,
    });
    yield {
    ...
    const selectedIndices = await (0, nova_1.filterBooks)(searchQuery, level || "beginner", searchResult.books);
    // If AI didn't select any, take top 5
    const indicesToUse = selectedIndices.length > 0
    ...
    for (let i = 0; i < selectedBooks.length; i++) {
        ...
        let resource = await Resource_1.Resource.findOne({
            source: book.source,
            externalId: book.id,
        ...
        try {
            const parseResult = await (0, client_1.fetchAndParse)({
            ...
            if (parseResult.success) {
                // Save to database
                resource = await Resource_1.Resource.findOneAndUpdate({ source: book.source, externalId: book.id }, {
                    source: book.source,
                    externalId: book.id,
                    title: book.title,
