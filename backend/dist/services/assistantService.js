...
exports.generateLessonAssistantReply = generateLessonAssistantReply;
function normalize(input) {
    return input.toLowerCase().trim();
}
function generateLessonAssistantReply(lesson, question) {
    ...
    if (q.includes("quiz") || q.includes("pass") || q.includes("score")) {
        return {
            answer: "To pass this lesson, explain concepts clearly, use concrete examples, and address each quiz prompt directly. The pass threshold is 70%.",
            contextTag: "quiz_guidance",
        };
    }
    if (q.includes("next") || q.includes("plan") || q.includes("practice")) {
        return {
            answer: "Use this sequence: review the lesson summary, draft short answers for each quiz question, then refine your wording to include concrete concepts.",
